...
from app.inspector.html_checks import (
    detect_embedded_json_markers, detect_job_links_from_html,
    detect_html_card_candidates, detect_pagination_hints,
)
...
def inspect_source(url: str) -> SourceInspectionResult:
    """
    Inspect a source URL and return SourceInspectionResult with
    detected type, confidence, signals, and recommendations.
    """
    notes: list[str] = []
    raw_signals: dict = {}
    resolved_url = url
    ...
    # 2. Check for block via HTTP errors stored in exception message
    ...
    notes.extend(pw_result.get("notes", []))

    raw_signals.update({
        "confirmed_api": bool(confirmed_apis),
        "suspected_api": bool(candidate_apis),
    ...
    return SourceInspectionResult(
        ...
        candidate_api_endpoints=confirmed_apis + candidate_apis,
